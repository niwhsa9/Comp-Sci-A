...
public class Mat {
    ...
    public Mat transpose() {
        Mat transposed = new Mat(col, row, new double[row*col]);
        int q = 0;
        for(int c = 0; c < col; c++) {
            for (int r = 0; r < row; r++) {
                transposed.data[q] = getElem(r, c);
                q++;
            }
        }
        return transposed;
    ...
    public double dot(Mat m) {
        Mat t = lmul(m);
        double c = 0;
        for(int i = 0; i < t.col * t.row; i++) c+= t.data[i];
        return c;
    }
    ...
    public Mat add(Mat m) {
        if(row != m.row && col != m.col) System.out.println("illegal mat mul attempted");
        Mat o = new Mat(row, col);
    ...
    public static Mat randomMat(int row, int col) {
