...
public class Boss extends Enemy{
	...
	public void paintComponent(Graphics g) {
		...
		for(int i = 0; i < missile.length; i++) missile[i].paintComponent(g);
	...
	public void update(double dt) {
		...
		if(time - prevHurt > 1.5) hurt = false;
		
		for(int i = 0; i < missile.length; i++) missile[i].update(dt);
		
		if(time - prevMissile > 3.0) {
			boolean flag = true;
			for(int i = 0; i < missile.length; i++) {
				if(missile[i] == null) break;
