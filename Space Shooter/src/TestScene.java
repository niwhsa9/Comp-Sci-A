...
public class TestScene extends Scene {
	...
	public void paintComponent(Graphics g) {
		...
		for(int i = 0 ; i < gameObjects.size(); i++) gameObjects.get(i).paintComponent(g2d);
	...
	public double smallest(Mat[] wall, Mat particle, double theta, Mat ship) {	
		...
		for(int i = 1; i < d.length; i++) {
			...
			//	System.out.println("choice " + d[i]+ " wall vec " + map.get(d[i]).data[0] + ", "  + map.get(d[i]).data[1] );
	...
	public Point2D traceToBorder(double x, double y, double theta) {
		...
		Mat bot = new Mat(3, 1, new double[] {(Constants.WindowDims.height + y)/Math.tan(theta) + x, Constants.WindowDims.height, 1});
		Mat left = new Mat(3, 1, new double[] {0, Math.tan(theta) * (0 - x) + y, 1 });
		Mat right = new Mat(3, 1,new double[] { Constants.WindowDims.width, Math.tan(theta) * (Constants.WindowDims.width - x) + y, 1});
		
		
	/*	
		System.out.println("top: " + particle.sub(top).getHomogenous().getMag() + " bot " + particle.sub(bot).getHomogenous().getMag() 
		+"left: " + particle.sub(left).getHomogenous().getMag() + " right " + particle.sub(right).getHomogenous().getMag() );
*/
		
		
		double border = smallest(new Mat[] {top, bot, left, right}, particle, theta, new Mat(3, 1, new double[] {x, y, 1}));
		...
		//	System.out.println("top x: " + b.getX());
			...
			b = new Point2D.Double((Constants.WindowDims.height + y)/Math.tan(theta) + x, Constants.WindowDims.height);
	...
	public void update(double dt) {
		...
		//ship.update(dt);
		for(int i = 0 ; i < gameObjects.size(); i++) gameObjects.get(i).update(dt);
		
		//System.out.println(stars.size());
		for(int i = 0; i < stars.size(); i++) {
			stars.get(i).theta = ship.theta + Math.PI;
			//Point2D q = traceToBorder(stars.get(i).x, stars.get(i).y, stars.get(i).theta); //remove
